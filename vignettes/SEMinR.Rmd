...
SEMinR supports out-of-sample prediction for PLS models, including models with interaction terms. This implements the PLSpredict procedure (Shmueli et al., 2016, 2019).

## Single holdout prediction

Use `predict()` on an estimated model with held-out test data:
...
predictions <- predict(model, testData = mobi[1:50, ])
predictions$predicted_items      # predicted indicator scores
predictions$item_residuals       # actual - predicted
```

All three interaction methods are supported for prediction: `two_stage`, `product_indicator`, and `orthogonal`. The prediction function automatically detects which method was used during estimation.

## Cross-validated prediction with `predict_pls()`

For k-fold or leave-one-out cross-validation (LOOCV), use `predict_pls()`:

```{r, eval=FALSE}
# 10-fold cross-validation
cv_results <- predict_pls(model, technique = predict_DA, noFolds = 10)
summary(cv_results)

# LOOCV (automatically parallelized)
loocv_results <- predict_pls(model, technique = predict_DA, noFolds = NULL)

# Parallel k-fold (useful for high k or interaction models)
cv_parallel <- predict_pls(model, technique = predict_DA, noFolds = 50, cores = 4)
```

The `technique` parameter controls how endogenous construct scores are predicted:

* `predict_DA` (Direct Antecedents): predicts each construct from its direct antecedent constructs only
* `predict_EA` (Earliest Antecedents): propagates predictions through the full structural model from the exogenous constructs

### Parallelization

Cross-validation is parallelized in two cases:

* **LOOCV** (`noFolds = NULL`): always runs in parallel, with cores auto-detected
* **k-fold with `cores` specified**: runs in parallel across the specified number of cores

This is particularly useful for high-k folds (e.g., 50 or 100) or interaction models where each fold requires re-estimation of interaction items.

*Note:* Parallel workers load the _installed_ version of seminr. If using development code, run `devtools::install()` before parallel prediction.
