...
1. Describe measurement model for each construct and its items, specifying interactions and higher order constructs:
```{r, eval=FALSE}
...
`constructs()` compiles the measurement model source-target matrix from the user specified construct descriptions described in the parameters. You must supply it with any number of individual _composite_, _reflective_, _interaction_, or _higher_composite_ constructs:
