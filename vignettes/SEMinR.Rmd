...
output:
  ...
  html_document:
    ...
    toc_depth: 2
vignette: |
  %\VignetteIndexEntry{SEMinR} %\VignetteEngine{knitr::rmarkdown} \usepackage[utf8]{inputenc}
---
...
* Extracts _ten Berge factor scores_ that have the same correlation pattern as the latent constructs
* Creates product-indicator _interactions_, or two-stage interactions using ten Berge scores from a CFA
* Defaults to *robust maximum-likelihood* (MLR) estimation to account for potential non-normality
...
structure <- relationships(
  ...
  paths(from = "Image*Expecttion", to = "Loyalty")
)
```
...
`composite()` or `reflective()` describe the measurement of aconstruct by its items.
...
For covariance-based SEM and CFA, you will want constructs to be _reflective_ common factors. If you have already have composite constructs or measurement models, you may use them for CBSEM/CFA after converting them to reflective versions. The `as.reflective()` function can convert either a single construct or an entire measurement model into reflective forms.
...
The `summary(boot_seminr_model)` function will return t_values and confidence intervals for direct structural paths in PLS models. However,
