...
Cross-validation runs sequentially by default. To enable parallel execution, specify the `cores` parameter:

```{r, eval=FALSE}
# Parallel LOOCV across 4 cores
loocv_parallel <- predict_pls(model, noFolds = NULL, cores = 4)

# Parallel 50-fold CV
cv_parallel <- predict_pls(model, noFolds = 50, cores = 4)
```

This is particularly useful for LOOCV or high-k folds where each fold requires re-estimation.
